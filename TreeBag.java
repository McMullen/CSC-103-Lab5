...
//
//Program:     TreeBag
//Description: TreeBag creates the framework for the binary search tree. The class
//             will create a link list designed to look like a tree when viewed
//             graphically. The nodes placement on the tree will be determined by
//             an integer connected to the node object. In this example, we will be
//             using the employee's ID number to determine which section of the
//             tree they belong in.
//----------------------------------------------------------------------------------
...
public class TreeBag
{
   ...
   {  
      if(p == null)  // tree is empty, target is not found 
         return p;
      int answer = target.compareTo(root.getData());
   	if(answer == -1) 
